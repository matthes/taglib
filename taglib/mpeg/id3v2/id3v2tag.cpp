...
#define kMusicBrainzOwnerId "http://musicbrainz.org"
...
String ID3v2::Tag::musicBrainzTrackId() const {

	UniqueFileIdentifierFrame *frame = UniqueFileIdentifierFrame::findByOwner(this, "http://musicbrainz.org");
...
String ID3v2::Tag::musicBrainzArtistId() const {

	if(!d->frameListMap["TXXX"].isEmpty()) {

		/// Find MusicBrainz Artist Id frame
		FrameList frameList = d->frameListMap["TXXX"];

		for(FrameList::ConstIterator iterator = frameList.begin(); iterator != frameList.end(); iterator++) {

			UserTextIdentificationFrame *frm = dynamic_cast<UserTextIdentificationFrame *>(*iterator);

			if(frm->description() == "MusicBrainz Artist Id") {

				/// Return last value from field list which is the actual musicbrainz Id
				return frm->fieldList().back();
			}
		}
	}
...
String ID3v2::Tag::musicBrainzAlbumId() const {

	/*FrameList fL = d->frameListMap["TXXX"];

	for(FrameList::ConstIterator iterator = fL.begin(); iterator != fL.end(); iterator++) {

		TextIdentificationFrame *frm = dynamic_cast<TextIdentificationFrame *>(*iterator);

		std::cout << frm->frameID() << " ->> ";// << " - \"" << (*iterator)->toString() << "\"" << std::endl;

		StringList fields = frm->fieldList();

		for(StringList::ConstIterator it = fields.begin(); it != fields.end(); it++) {

				std::cout << it->data(String::UTF8) << std::endl;
		}
	...
	if(!d->frameListMap["TXXX"].isEmpty()) {
		...
		/// Find MusicBrainz Album Id frame
		FrameList frameList = d->frameListMap["TXXX"];
		...
		for(FrameList::ConstIterator iterator = frameList.begin(); iterator != frameList.end(); iterator++) {
			...
			if(frm->description() == "MusicBrainz Album Id") {
...
String ID3v2::Tag::acustidFingerprint() const {
	...
	if(!d->frameListMap["TXXX"].isEmpty()) {
		...
		for(FrameList::ConstIterator iterator = frameList.begin(); iterator != frameList.end(); iterator++) {
			...
			if(frm->description() == "Acoustid Fingerprint") {
...
void ID3v2::Tag::setMusicBrainzTrackId(const String &s) {

	if(s.isEmpty()) {

		removeUniqueFileIdentifierFrameWithOwner(kMusicBrainzOwnerId);
	}
	else {

		setUniqueFileIdentifierFrame(kMusicBrainzOwnerId, s);
	}
}

void ID3v2::Tag::setMusicBrainzArtistId(const String &s) {

	removeUniqueFileIdentifierFrameWithOwner(kMusicBrainzOwnerId);

	setUserTextFrame("MusicBrainz Artist Id", s);
}

void ID3v2::Tag::setMusicBrainzAlbumId(const String &s) {
	...
	setUserTextFrame("MusicBrainz Album Id", s);
}

void ID3v2::Tag::setAcustidFingerprint(const String &s) {

	setUserTextFrame("Acoustid Fingerprint", s);
}
...
void ID3v2::Tag::setUniqueFileIdentifierFrame(const String &ownerId, const String &ufId) {

	const char *valueString = ufId.toCString(true);

	ByteVector data = ByteVector(valueString, strlen(valueString));

	UniqueFileIdentifierFrame *frame = UniqueFileIdentifierFrame::findByOwner(this, ownerId);

	if(!frame) {

		frame = new UniqueFileIdentifierFrame(ownerId, data);

		addFrame(frame);
	}
	else {

		frame->setIdentifier(data);
	}
}

void ID3v2::Tag::removeUniqueFileIdentifierFrameWithOwner(const String &ownerId) {
...
void ID3v2::Tag::setUserTextFrame(const ByteVector &id, const StringList &values) {
	...
	if(!d->frameListMap[id].isEmpty()) {
		...
		UserTextIdentificationFrame *frame = (UserTextIdentificationFrame *)d->frameListMap[id].front();

		frame->setText(values);
	}
	else {
		const String::Type encoding = d->factory->defaultTextEncoding();
		TextIdentificationFrame *f = new UserTextIdentificationFrame(id, values, encoding);
		addFrame(f);
	}
}
